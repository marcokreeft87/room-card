...
console.info('%c ROOM-CARD %c 1.2.5', 'color: cyan; background: black; font-weight: bold;', 'color: darkblue; background: white; font-weight: bold;');
let RoomCard = class RoomCard extends lit_1.LitElement {
    ...
    set hass(hass) {
        ...
        if (hass && this.config) {
            ...
            this._refCards = (_e = this.config.cards) === null || _e === void 0 ? void 0 : _e.map((config) => {
    ...
    renderEntity(stateObj, config) {
        ...
        ">
            <span>${(0, entity_1.entityName)(stateObj, config)}</span>
...
const getEntityIds = (config) => {
    var _a, _b;
    return [config.entity, config.entities, config.info_entities]
